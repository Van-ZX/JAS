...
IMAGE_SIZE = 512
BATCH_SIZE = 1
COVER_PATH = '/public/g16/BossBase512'
OUTPUT_PATH = '/public/fanzexin/adv_dejoin/new_stego_0.4'
payload = 0.4
...
def test(model, device, loader):
    ...
    for idx, sample in enumerate(loader):
        ...
        tmp = sign_grad[1, :, :].cpu().detach().numpy().copy().reshape(IMAGE_SIZE, IMAGE_SIZE)
        ...
        if correct: # 分析器能准确分类
            ...
            for i in range(count_sub):
                b_cover = cover[IMAGE_SIZE*i//count_sub : IMAGE_SIZE*(i+1)//count_sub, :]
                b_rho = rhoini[:, IMAGE_SIZE*IMAGE_SIZE*i//(2*count_sub) : IMAGE_SIZE*IMAGE_SIZE*(i+1)//(2*count_sub)]
                b_steg = embed(b_cover, b_rho, msg)
                stego_ite[IMAGE_SIZE*i//count_sub : IMAGE_SIZE*(i+1)//count_sub, :] = b_steg
            ...
            while ZETA < MAX_ZETA + 0.00001 and correct:
                ...
                #     ipdb.set_trace()
                    ...
                    b_cover = cover[IMAGE_SIZE*i//count_sub : IMAGE_SIZE*(i+1)//count_sub, :]
                    b_sign = sgn[:, IMAGE_SIZE*IMAGE_SIZE*i//(2*count_sub) : IMAGE_SIZE*IMAGE_SIZE*(i+1)//(2*count_sub)]
                    b_rho = rhoini[:, IMAGE_SIZE*IMAGE_SIZE*i//(2*count_sub) : IMAGE_SIZE*IMAGE_SIZE*(i+1)//(2*count_sub)]
                    rhonew = adjust_cost(b_rho, b_sign, ALPHA)
                    ...
                    model.zero_grad()
                    loss.backward()
                    sign_grad = newdata.grad.data.sign()
                    tmp = sign_grad[1, :, :].cpu().detach().numpy().copy().reshape(IMAGE_SIZE, IMAGE_SIZE)
                    sgn = np.zeros((2, IMAGE_SIZE * IMAGE_SIZE // 2))
                    shape = tmp.reshape(1, -1).copy()
                    sgn[0, :] = shape[:, 0: : 2]
            ...
            #     steg = Image.fromarray(coefS).convert('L')
            #     steg.save(os.path.join(OUTPUT_PATH, name[0]))
            #     with open('defeat.txt', 'a') as file:
            #         file.write(name[0] + '\n')
...
class MyDataset(Dataset):
    def __init__(self, partition, transform):
        ...
        self.stego_dir = '/public/fanzexin/dejoin_matlab/stego_HILL_dejoin_payload0.4'
        ...
        if (partition == 0):
            self.cover_list = self.covers_list_all[:100]
...
if __name__ == '__main__':
    path = '/public/fanzexin/adv_dejoin/stego_hill_dejoin_0.4_AUG/model_params.pt'
